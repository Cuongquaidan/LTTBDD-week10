...
import { createNote, updateNote } from './features/notesSlice';
...
const Add = ({ navigation, route }) => {
  ...
  const handleAdd = async () => {
    ...
    try {
      ...
      const response = await dispatch(createNote(newNote)).unwrap(); // Use unwrap to get the response

      navigation.navigate('Manage', { notes: [...notes, response] });
    } catch (err) {
      console.error(err);
    }
  };
  ...
  const handleUpdate = async () => {
    ...
    try {
      const updatedNote = { id: note.id, content };
      const response = await dispatch(updateNote(updatedNote)).unwrap();

      const updatedNotes = notes.map(item => (item.id === note.id ? response : item));
  ...
  return (
    <SafeAreaView style={{ padding: 20, alignItems: "center" }}>
      ...
      <View style={{ marginTop: 50, alignItems: "center", gap: 30 }}>
        ...
        />
        <Pressable
          style={{ backgroundColor: "#00BDD6", width: 160, height: 50, borderRadius: 10, alignItems: "center", justifyContent: "center", paddingBottom: 10 }}
          onPress={note ? handleUpdate : handleAdd}
        >
          <Text style={{ fontSize: 30, color: "white" }}>Finish</Text>
        </Pressable>
        <Image source={require("./assets/note.png")} style={{ width: 200, height: 200, marginTop: 50 }} />
      </View>
    </SafeAreaView>
  );
};
