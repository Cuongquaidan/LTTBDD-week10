...
const Manage = ({ navigation }) => {
  useFetchNotes(); // Fetch notes on component mount
          ...
          {notes.map(note => (
            <View key={note.id} style={{ width: 300, borderRadius: 10, borderWidth: 1, flexDirection: "row", justifyContent: "space-between", padding: 10, backgroundColor: "#ddd" }}>
              ...
              <View style={{ flexDirection: "row", gap: 10 }}>
                ...
                <Text style={{ color: "orange" }} onPress={() => navigation.navigate("Add", { note })}>Update</Text>
              </View>
            </View>
          ))}
        </View>
        <Pressable onPress={() => navigation.navigate("Add")} style={{ backgroundColor: "#00BDD6", width: 50, height: 50, borderRadius: 25, alignItems: "center", justifyContent: "center", paddingBottom: 10 }}>
